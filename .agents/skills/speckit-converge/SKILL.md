...
of the code relative to the feature's artifacts â€” no git, no branch comparison, no history.
...
- rewrite, renumber, reorder, or delete any existing task (including tasks from a prior
  Convergence phase);
- modify, create, or delete any application code â€” completing the appended tasks is the
...
- Success Criteria (SC-###) â€” include only items requiring buildable work; exclude
...
- **Code-scope map**: from the file paths named in `plan.md` and `tasks.md`, plus a keyword
  ...
  components in scope for assessment. Bound the assessment to these â€” do **not** infer
...
- **`unrequested`**: the code contains work not called for by the spec, plan, or tasks
  (surfaced for awareness â€” converge does **not** delete code, it only appends a task to
...
- Constitution principles checked (or "skipped â€” template")
...
3. Emit one checklist item per actionable finding, ordered CRITICAL/HIGH first, assigning
   zero-padded IDs `T{M+1:03d}, T{M+2:03d}, â€¦`:
...
4. Never reuse or renumber existing IDs. If a prior Convergence phase exists, add a new,
   separately-numbered one below it â€” do not touch the old one.

**If there are no actionable findings** (`converged` outcome):

- Do **not** modify `tasks.md` at all â€” no empty phase header.
- Report: **"âœ… Converged â€” the implementation satisfies the spec, plan, and tasks."**
