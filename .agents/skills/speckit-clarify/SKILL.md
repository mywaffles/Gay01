...
9. **Re-validate Spec Quality Checklist** (if it exists):
   ...
   - If it exists:
     ...
     2. Identify all GitHub task-list checkbox lines â€” lines matching `- [ ]`, `- [x]`, or `- [X]` (case-insensitive, tolerant of leading whitespace for nested items) outside of code fences. Ignore all other content (headings, notes, non-checkbox bullets, metadata).
     ...
     6. Save the updated checklist file. **Only toggle the `[ ]`/`[x]` marker portion of checkbox lines whose state changed.** All other file content â€” headings, metadata, notes, line ordering, whitespace â€” must remain unchanged to avoid noisy diffs.
     ...
     8. Record the before/after pass counts as checked/total checkbox items (e.g., "12/16 â†’ 15/16 items passing").
...
- Spec quality checklist status (if `FEATURE_DIR/checklists/requirements.md` was re-validated): show before/after pass counts (e.g., "Spec Quality Checklist: 12/16 â†’ 15/16 items passing") and list any items that changed state â€” both newly checked (unchecked â†’ checked) and any regressions (checked â†’ unchecked). If any items remain unchecked, list them as areas needing attention.
